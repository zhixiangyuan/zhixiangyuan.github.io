...
AQS 实现的是对与线程的管理，当尝试获取锁失败的时候，会将当前线程加入到等待队列里面去，当有任务释放锁时会从等待队列里面激活一个线程继续向下执行。

内部的锁分为共享模式和独占模式，在独占模式时，锁只能被单个线程持有，在锁释放后会会从队列中激活一个线程向下执行。在共享模式时，锁可以被多个线程持有，而在锁被释放时，会激活所有的节点。
